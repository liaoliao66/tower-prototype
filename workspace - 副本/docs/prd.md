...
author: 浙江中能工程检测有限公司
status: 内部文件 · 机密
description: 本 PRD 面向 AI 原型与研发交付，融合飞书《产品评审》及存量铁塔原始记录附表实例；V3.2 强化检测任务模板「共性抽象 + 通用引擎」表述；V3.3 对齐飞书《检测任务》对 PC 端检测数据复核留痕与 APP 端开启任务及分模块填报、语音备忘、拍照识别等能力。覆盖 PC 管理后台 + 移动端现场作业双端。
...
<center>V3.3 | 2026年5月 | 浙江中能工程检测有限公司</center>

# 文档控制信息

| 项 | 内容 |
| --- | --- |
| 文档名称 | 通信铁塔检测系统产品需求文档（PRD）V3.3 |
| 版本号 | V3.3 |
| 编制日期 | 2026年5月14日 |
| 编制单位 | 浙江中能工程检测有限公司 |
| 密级 | 内部文件 · 机密 |
| 适用范围 | PC 管理后台 + 移动端 APP（H5/小程序） |
...
本产品为面向通信铁塔结构安全检测行业的 **B 端一体化业务系统**，以 **合同 → 工程 → 订单 → 检测任务（检测单位视角）→ 现场检测 → 报告编制与审核 → 交付归档** 为主价值链。PC 端承担主数据、合同与商务、派单与进度管控、模板与权限配置、报告与审核流；移动端面向一线检测人员，强调 **弱网下可用、数据自动保存、提交后锁定**。

与 V2.0 相比，V3.0 / V3.1 明确 **外协/多检测单位** 协作模型：订单可先派至检测单位，再由检测单位内部 **派任务到具体人员**，并支持改派、拒绝、催办等运营动作；V3.2 起检测任务以 **模板引擎 + 飞书附表实例包** 对齐（§3.8.1）。**V3.3** 起，现场执行侧对齐飞书《检测任务》（`source_detection_task_flow`）：**PC 侧「检测任务模块」配置决定 APP 在对应塔型下须采集的数据项**；APP 以 **开启任务向导 + 分大模块表单** 承载填报，并补充 **语音备忘、拍照识别、提交前后规则** 与 PC 侧 **审核改数留痕 + 记录版本** 产品要求（§3.6.3～3.6.4、§3.11）。
...
| 现场检测人员 | 现场数据采集、照片与记录、提交/草稿 | APP | 本人任务及关联工程数据 |
...
| 任务模板/报告模板 | /task-template/*, /report-template/* | 模板中心 | 配置 | 管理员 |
...
| 模板中心 | 任务与报告规范化 | 任务模板包 / 报告模板 | 版本与发布；任务模板引擎详见 §3.8.1 |
| 基础数据 | 主数据与字典 | 检测单位/设备/站点/类型字典 | 多租户隔离 |
| 移动端 | 现场执行 | 开启向导/分模块填报/语音备忘/拍照识别/草稿提交/同步 | 弱网可用；§3.11 |
...
| 任务 Task | 检测单位ID、指派人员ID、拒绝/撤单原因、审核状态细分；**template_package_id、template_version、template_snapshot_json（模板包快照）**；**tower_segment_count_app**（APP 向导确认的塔段数，写入后与快照一致） | 状态与 §2.4.2 对齐；快照与 §3.8.1 对齐 |
...
✅ **PRD 与交互流程已生成**：`/workspace/docs/prd.md`
