...
- **消息触达**：提醒时间到达后，按「消息模板配置」渲染标题与正文，写入**消息中心**（类型：合同提醒）；**不进入工作台待办**。模板场景：对上合同-收款/付款提醒、对下合同-付款提醒；支持转义字段含合同金额（含税/不含税）、当前累计收款金额、当前累计付款金额等。
- **前置条件**：合同已存在。
- **数据权限**：合同部；财务角色若启用只读。
- **审批**：合同主档审批走**基础平台**流程；业务系统展示审批状态并提供「提交审批 / 去审批（平台）」入口。
